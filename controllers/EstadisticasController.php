...
class EstadisticasController extends ActiveRecord
{
   ...
   public static function buscarPersonalMasAsignadoAPI(){
       try {
           $sql = "SELECT u.usuario_nom1 || ' ' || u.usuario_ape1 as personal, COUNT(cp.comision_personal_id) as asignaciones
                   FROM avpc_comision_personal cp
                   INNER JOIN avpc_usuario u ON cp.usuario_id = u.usuario_id
   ...
   public static function buscarUsuariosCreadoresAPI(){
       try {
           $sql = "SELECT u.usuario_nom1 || ' ' || u.usuario_ape1 as creador, COUNT(c.comision_id) as comisiones_creadas
                   FROM avpc_comision c
                   INNER JOIN avpc_usuario u ON c.comision_usuario_creo = u.usuario_id
   ...
   public static function buscarComisionesActivasAPI(){
       try {
           $sql = "SELECT 
                   ...
                   FROM avpc_comision c
                   LEFT JOIN avpc_comision_personal cp ON c.comision_id = cp.comision_id AND cp.comision_personal_situacion = 1
       ...
       try {
           $sql = "SELECT 
                   ...
                   LEFT JOIN (
                       ...
                       FROM avpc_comision_personal cp
                       INNER JOIN avpc_comision c ON cp.comision_id = c.comision_id
   ...
   public static function buscarResumenGeneralAPI(){
       try {
           $sql = "SELECT 
                       (SELECT COUNT(*) FROM avpc_comision WHERE comision_situacion = 1) as total_comisiones,
                       (SELECT COUNT(*) FROM avpc_comision WHERE comision_situacion = 1 AND comision_estado = 'PROGRAMADA') as comisiones_programadas,
                       (SELECT COUNT(*) FROM avpc_comision WHERE comision_situacion = 1 AND comision_estado = 'EN_CURSO') as comisiones_en_curso,
                       (SELECT COUNT(*) FROM avpc_comision WHERE comision_situacion = 1 AND comision_estado = 'COMPLETADA') as comisiones_completadas,
                       (SELECT COUNT(*) FROM avpc_usuario WHERE usuario_situacion = 1) as total_usuarios,
                       (SELECT COUNT(*) FROM avpc_rutas WHERE ruta_situacion = 1) as total_rutas";
